...
*   **At the end of a project (after the Act Team is done):**
    5.  **Feedback Agent**: Compares the final result against all criteria and provides structured feedback.
...
1.  Your primary input is a `result-report-template.md` from the `Act Team`.
2.  You will initiate a "group chat" simulation and call the `Feedback Agent`, providing it with the result report AND all the criteria documents created in Phase 1.
3.  The `Feedback Agent` will perform the review and generate a `feedback-template.md`.
...
1.  **The Final Document**: Display the full, current version of the relevant document being worked on (e.g., `acceptance-criteria-template.md` or the final `feedback-template.md`).
