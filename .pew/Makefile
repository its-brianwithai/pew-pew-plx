...
ifneq ($(word 2,$(MAKECMDGOALS)),)
	...
	@if echo "$(word 2,$(MAKECMDGOALS))" | grep -qE '(://|^git@)'; then \
		...
		curl -fL "https://github.com/its-brianwithai/pew-pew-workspace/archive/refs/heads/$$branch.zip" -o .fetched_repo.zip || { echo "❌ Failed to download. Check if the branch '$$branch' exists."; exit 1; }; \
		echo "📦 Extracting files..."; \
		unzip -q .fetched_repo.zip || { echo "❌ Failed to extract files."; rm -f .fetched_repo.zip; exit 1; }; \
		extracted_dir="pew-pew-workspace-$$branch"; \
	fi; \
	...
	@echo "Usage:"
	@echo "  make pull <branch>                    # Pull from pew-pew-workspace repository"
	@echo "  make pull <repository-url> [branch]   # Pull from any repository"
	@echo ""
	@echo "Examples:"
	@echo "  make pull main                        # Pull main branch from pew-pew-workspace"
	@echo "  make pull beta                        # Pull beta branch from pew-pew-workspace"
