...
async function findConfigUpwards(startDir) {
  let dir = startDir;
  while (true) {
    const candidatePlx = join(dir, 'plx.yaml');
    const contentPlx = await readIfExists(candidatePlx);
    if (contentPlx) return { path: candidatePlx, content: contentPlx };
    const candidateCfg = join(dir, 'config.yaml');
    const contentCfg = await readIfExists(candidateCfg);
    if (contentCfg) return { path: candidateCfg, content: contentCfg };
...
export async function loadConfig() {
  ...
  if (!yamlContent) {
    ...
    const userPlxPath = join(userRootDir, 'plx.yaml');
    yamlContent = await readIfExists(userPlxPath) || '';
    if (!yamlContent) {
      const userCfgPath = join(userRootDir, 'config.yaml');
      yamlContent = await readIfExists(userCfgPath) || '';
    }
  }

  // 3) fallback to bundled default
  if (!yamlContent) {
    const defaultPlx = join(__dirname, '..', 'plx.yaml');
    yamlContent = await readIfExists(defaultPlx);
    if (!yamlContent) {
      const defaultCfg = join(__dirname, '..', 'config.yaml');
      yamlContent = await readIfExists(defaultCfg);
    }
